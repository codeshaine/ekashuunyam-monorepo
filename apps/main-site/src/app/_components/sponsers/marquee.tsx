import { Sponsers, SponsersPriority } from "@/lib/data/sponsers";
...
export const SponsersMarquee = ({
    ...
    let high = priority === SponsersPriority.High;
    return (
      <Marquee
          ...
          {data.map((brand) => (
            ...
            >
              ...
              <div className="rounded-b-xl px-1 text-black transition-all duration-300 ease-in group-hover:bg-white">
                <p className={`mt-2 font-bold ${high ? "text-xl" : "text-sm md:text-base"}`}>
                  {brand.name}
                </p>
